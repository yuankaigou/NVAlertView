...
@class NVTimerDisplay;

@interface NVButton : UIButton

typedef void (^NVActionBlock)(void);
typedef BOOL (^NVValidationBlock)(void);
typedef NSDictionary* (^CompleteButtonFormatBlock)(void);
typedef NSDictionary* (^ButtonFormatBlock)(void);

// Action Types
typedef NS_ENUM(NSInteger, NVActionType)
{
    NVNone,
    NVSelector,
    NVBlock
 ...
 */
@property (nonatomic) NVTimerDisplay *timer;
